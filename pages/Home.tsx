...
export const Home: React.FC<HomeProps> = ({ templates, settings }) => {
  ...
  const isFormValid = useMemo(() => {
    const allTextsFilled = selectedTemplate.textFields
      .filter(f => f.isVisible !== false)
      .every(f => userTexts[f.id] && userTexts[f.id].trim() !== '');
    const allPhotosFilled = selectedTemplate.photoFields
      .filter(f => f.isVisible !== false)
      .every(f => userPhotos[f.id]);
    return allTextsFilled && allPhotosFilled;
  }, [selectedTemplate, userTexts, userPhotos]);

  const downloadHD = async () => {
    if (!isFormValid) {
      setErrorMsg("All visible fields are required. Please upload your photo and fill in your details.");
      return;
    }
      ...
      <div className="w-full md:w-96 p-6 bg-white shadow-xl z-10 overflow-y-auto flex flex-col border-r border-slate-100">
        ...
        <section className="space-y-6 flex-1">
          ...
          <div className="space-y-4">
            ...
            {selectedTemplate.textFields.filter(f => f.isVisible !== false).map(f => (
            ...
            {selectedTemplate.photoFields.filter(f => f.isVisible !== false).map(f => (
              <div key={f.id} className="space-y-1.5">
                <label className="text-[10px] font-black uppercase text-slate-600 ml-1 tracking-wider">{f.label}</label>
